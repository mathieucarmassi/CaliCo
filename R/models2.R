...
model.class <- R6Class(classname = "model.class",
                       public = list(
                         ...
                         initialize = function(code=NA,X=NA,Yexp=NA,model=NA)
                         {
                           ...
                           self$model   <- model
                           self$Yexp    <- Yexp
                           self$n       <- length(self$Yexp)
                           self$X       <- self$extract_X(X)
                           self$d       <- ncol(self$X)
                           ##########################################################
                           ...
                           ##########################################################
                           private$checkModels()
                           private$checkCode() # for model1 and model3 only where the code has to be defined
                           #private$checkOptions()
                           self$f.variables <- private$testOnForcingVar()
                         },
                         active = list(
...
## Function that import the forced variables to the proper format
model.class$set("public","extract_X",
                function(X)
                {
