...
#' @export
prior.class <- R6Class("prior.class",
                 public = list(
                   type.prior = NULL,
                   opt.prior  = NULL,
                   mean       = NULL,
                   ...
                   n.prior    = NULL,
                   density    = NULL,
                   l          = NULL, #list of prior
                   initialize = function(type.prior=NA,mean=NA,s2=NA,shape=NA,scale=NA,alpha=NA,beta=NA,binf=NA,bsup=NA){
                     browser()
                     if (length(type.prior) == 1)
                     {
                       ...
                       switch (type.prior,
                         gauss = {
                           self$mean <- mean
                           self$s2   <- s2
                           self$sd   <- sqrt(self$s2)
                           self$binf <- self$mean - 4*sqrt(self$s2)
                           self$bsup <- self$mean + 4*sqrt(self$s2)
                         },
                         gamma = {
                           self$shape <- shape
                           self$scale <- scale
                         ...
                         invGamma = {
                           ...
                           self$beta  <- beta
                           self$binf  <- 0
                           self$bsup  <- 6*self$beta/(self$alpha+1)
                         }
                       )
                       self$density   <- self$select.prior(type.prior)
                     } else {stop("multiple prior definition is not available")}
                   },
                   select.prior = function(type.prior,...) {
                     switch(type.prior,
                            gauss = {
                              density = function(x=seq(self$binf,self$bsup,length.out = 100),log=TRUE)
                              {
                                if (log == FALSE)
                                {
                                  return(1/(sqrt(2*pi*self$sd))*exp(-1/(2*self$sd)*(x-self$mean)^2))
                                } else
                                {
                                  return(log(1/(sqrt(2*pi*self$sd))*exp(-1/(2*self$sd)*(x-self$mean)^2)))
                                }
                              }
                            },
                            gamma={
                              density = function(x=seq(0,self$bsup,length.out = 100))
                              {
                                if (self$log == FALSE)
                                {
                            ...
                            invGamma={
                              density = function(x=seq(0,self$bsup,length.out = 100))
                              {
                                if (self$log == FALSE)
                                {
                                  return(self$beta^self$alpha/Gamma(self$alpha)*x^(-1-self$alpha)*exp(-self$beta/x))
                                } else
                                {
                                  return(self$alpha*log(self$beta)-log(Gamma(self$alpha))-(self$alpha+1)*log(x)-self$beta/x)
                                }
                              }
                            },
                            unif={
                              density = function(x=seq(self$binf,self$bsup,length.out = 100))
                              {
                                ...
                                if (self$log == FALSE){return(self$y)} else{return(log(self$y))}
                              }
                            }
                     )
                     return(density)
...
prior.class$set("public","print",
                function()
                {
                  cat("Call:\n")
                  print(self$type.prior)
                  cat("\n")
                  cat("Summary of 100 random samples in the distribution:\n")
                  print(summary(self$density()))
                  cat("\n")
                })

prior.class$set("public","plot",
                function()
                {
                  browser()
                  y <- self$density()
                  df <- data.frame(y=y,x=1:length(y),type="prior")
                  p <- ggplot(df,aes(x=x,fill=type,color=type)) +
