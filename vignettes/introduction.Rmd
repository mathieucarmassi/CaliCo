...
Yexp <- code(X,10.8)+rnorm(n,0,sqrt(0.01))
Yt <- code(X,11)
...
theta.prior <- rnorm(100,11,sqrt(0.01))
sigma.prior <- rgamma(100,1,0.01)

Y_prior <- matrix(nr=100,nc=n)
for (i in 1:100){
  Y_prior[i,] <- code(X,theta.prior[i])+rnorm(n,0,sqrt(sigma.prior[i]))
}
...
Yexp <- code(t,10.8)+rnorm(n,0,sqrt(0.01))
...
opt.emul <- list(p=1,n.emul=200,type="matern5_2",binf=binf,bsup=bsup,DOE=NULL)
...
opt.estim1=list(Ngibbs=3000,Nmh=10000,thetaInit=c(11,0.01),k=c(0.1,0.1),sig=diag(2),Nchains=1,burnIn=2500)

# Generate the prior densities
pr1 <- prior(type.prior=c("gaussian","gamma"),opt.prior=list(c(10.5,4),c(2,0.01)))
...
mdfit2 <- calibrate(model2,pr1,opt.estim1)
```

`mdfit1` and `mdfit2` are two `R6` objects from `estim.class` which contain also two main methods (`$plot()` and `$print()`)
