...
*Note that `CaliCp` uses the package `DiceKrigging` to run the estimation of the Gaussian process.*
```{r, echo=FALSE}
library(CaliCp)
```                
...
In Bayesian calibration a prior distribution is updated in a posterior distribution thanks to the likelihood. Define the prior distribution in `CaliCp` is done as the following chunk.
...
To run a Bayesian calibration, the usefull function in `CALICO` is `calibrate`.
