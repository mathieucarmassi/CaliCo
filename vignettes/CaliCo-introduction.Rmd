...
opt.estim1=list(Ngibbs=2000,Nmh=4000,thetaInit=c(11,0.01),
                r=c(0.3,0.3),sig=diag(2),Nchains=1,burnIn=1000)
opt.estim2=list(Ngibbs=2000,Nmh=4000,thetaInit=c(11,0.01),
                r=c(0.3,0.3),sig=diag(2),Nchains=1,burnIn=1000)
...
opt.estim2=list(Ngibbs=1000,Nmh=3000,thetaInit=c(11,0.5,0.01,0.01),
                r=c(0.3,0.3),sig=diag(4),Nchains=1,burnIn=1000)

# Generate the prior densities
pr2 <- prior(type.prior=c("gaussian","gamma","unif","gamma"),
             opt.prior=list(c(11,0.5),c(1,0.5),c(0,1),c(2,0.01)))
