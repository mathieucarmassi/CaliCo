...
model2code <- model(code,X,Yexp,"model2",
                    ...
                    opt.emul=list(p=1,n.emul=100,binf=8,bsup=13))
```
...
In that case, one is free to generate a DOE appart from the function `model`. Let us consider, for the following, that the DOE is generated by the package `DiceDesign`, is a maximin LHS of $50$ points and is called `myDOE`.


```{r, echo=FALSE}
myDOE <- DiceDesign::lhsDesign(50,2)$design
...
```{r, echo=TRUE}
model3 %<% list(theta=11,thetaD=c(2,0.5),var=0.1)
model4 %<% list(theta=11,thetaD=c(2,0.5),var=0.1)
```
...
mdfit1 <- calibrate(model1,pr1,
                    opt.estim = list(Ngibbs=2000,Nmh=4000,thetaInit=c(11,0.01),
                                     r=c(0.3,0.3),sig=diag(2),Nchains=1,burnIn=1000))
```
...
```{r, echo=TRUE, eval=FALSE,fig.width=5,fig.height=4,fig.align='center'}
plot(fr,c(t,x.new))
```

```{r, echo=FALSE,fig.width=5,fig.height=4,fig.align='center'}
plot(fr,c(t,x.new)) + theme(legend.position = c(0.32,0.25))
```

The blue line corresponds to the predicted values and the credibility interval at $95\%% is also given.
